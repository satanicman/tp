...
{if $PS_SC_TWITTER || $PS_SC_FACEBOOK || $PS_SC_GOOGLE || $PS_SC_PINTEREST}
	...
	<p id="soc-share-block" class="socialsharing_product list-inline no-print">
		{if $PS_SC_TWITTER}
			<button id="soc-tw" data-type="twitter" type="button" class="btn btn-default btn-twitter social-sharing">
				<i class="icon-twitter"></i> {l s="Tweet" mod='socialsharing'}
				<!-- <img src="{$link->getMediaLink("`$module_dir`img/twitter.gif")}" alt="Tweet" /> -->
			</button>
		{/if}
		{if $PS_SC_FACEBOOK}
			<button id="soc-fb" data-type="facebook" type="button" class="btn btn-default btn-facebook social-sharing">
		...
		{/if}
		{if $PS_SC_GOOGLE}
			<button id="soc-g" data-type="google-plus" type="button" class="btn btn-default btn-google-plus social-sharing">
