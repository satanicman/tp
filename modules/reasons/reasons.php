...
class REASONS extends Module
{
	public function __construct()
	{
		$this->name = 'reasons';
		$this->description = 'reasons why us';
		$this->tab = 'front_office_features';
		$this->version = '1.1.1';
		$this->author = 'emotionLoop';
		$this->need_instance = 0;
		$this->ps_versions_compliancy = array('min' => '1.5', 'max' => '1.6');
		$this->monolanguage_content = 'Reasons_MONOLANGUAGE';
		$this->text_editor_content = 'Reasons_TEXTEDITOR';
		$this->content_wrapper = 'Reasons_CONTENTWRAPPER';
		$this->content_wrapper_class = 'Reasons_CONTENTWRAPPER_CLASS';
		$this->content_wrapper_id = 'Reasons_CONTENTWRAPPER_ID';
		$this->bootstrap = true;
		$this->_html = '';
		...
		$this->displayName = $this->l('reasons');
		$this->description = $this->l('reasons why us');
	...
	public function install()
	{
		...
		if (!parent::install() || !$this->registerHook('header') || !$this->registerHook('footer') || !REASONSModel::createTables())
			return false;
	...
	public function addFilesToTemplate($path = null)
	{
		...
		$files_data = REASONSModel::getFilesInUse( $this->selected_store_id, $this->selected_language_id );
	...
	{
		$content_query = REASONSModel::getContent( $this->selected_store_id, $this->selected_language_id );
		$pre_content = '';
		$pos_content = '';
		...
		if (Tools::isSubmit('submit'.$this->name))
		{
				...
				&& (int)Configuration::get('PS_LANG_DEFAULT') == $this->selected_language_id ))
			{
				...
				if (Tools::getValue('content_text') !== false && $this->ignore_changes_content_changes == false)
					REASONSModel::setContent( Tools::getValue('content_text'), $this->selected_store_id, $this->selected_language_id );

				//store the files to be used
				if (Tools::getValue('headerFiles') !== false)
					REASONSModel::setFiles(
	...
	public function displayForm()
	{
		...
		$helper->fields_value['headerFiles[]'] = $this->processSelectFilesForMultiselect(
																REASONSModel::getFilesInUse( $this->selected_store_id, $this->selected_language_id )
																);
...
class REASONSModel extends ObjectModel
{
	...
	public static function createTables()
	{
		...
		return ( REASONSModel::createFilesTable()
				&& REASONSModel::createContentTable());
	}
