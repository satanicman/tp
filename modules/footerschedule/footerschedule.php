...
class FOOTERSCHEDULE extends Module
{
	public function __construct()
	{
		$this->name = 'footerschedule';
		$this->description = 'footerSchedule';
		$this->tab = 'front_office_features';
		$this->version = '1.1.1';
		$this->author = 'emotionLoop';
		$this->need_instance = 0;
		$this->ps_versions_compliancy = array('min' => '1.5', 'max' => '1.6');
		$this->monolanguage_content = 'Footerschedule_MONOLANGUAGE';
		$this->text_editor_content = 'Footerschedule_TEXTEDITOR';
		$this->content_wrapper = 'Footerschedule_CONTENTWRAPPER';
		$this->content_wrapper_class = 'Footerschedule_CONTENTWRAPPER_CLASS';
		$this->content_wrapper_id = 'Footerschedule_CONTENTWRAPPER_ID';
		$this->bootstrap = true;
		$this->_html = '';
		...
		$this->displayName = $this->l('footerSchedule');
		$this->description = $this->l('footerSchedule');
	...
	public function install()
	{
		...
		if (!parent::install() || !$this->registerHook('header') || !$this->registerHook('footer') || !FOOTERSCHEDULEModel::createTables())
			return false;
	...
	public function uninstall()
	{
		$this->_clearCache('template.tpl');
		if (!parent::uninstall()
			|| !FOOTERSCHEDULEModel::DropTables())
		{
	...
	public function addFilesToTemplate($path = null)
	{
		...
		$files_data = FOOTERSCHEDULEModel::getFilesInUse( $this->selected_store_id, $this->selected_language_id );
	...
	{
		$content_query = FOOTERSCHEDULEModel::getContent( $this->selected_store_id, $this->selected_language_id );
		$pre_content = '';
		$pos_content = '';
		...
		if (Tools::isSubmit('submit'.$this->name))
		{
				...
				&& (int)Configuration::get('PS_LANG_DEFAULT') == $this->selected_language_id ))
			{
				...
				if (Tools::getValue('content_text') !== false && $this->ignore_changes_content_changes == false)
					FOOTERSCHEDULEModel::setContent( Tools::getValue('content_text'), $this->selected_store_id, $this->selected_language_id );

				//store the files to be used
				if (Tools::getValue('headerFiles') !== false)
					FOOTERSCHEDULEModel::setFiles(
	...
	public function displayForm()
	{
		...
		$helper->fields_value['headerFiles[]'] = $this->processSelectFilesForMultiselect(
																FOOTERSCHEDULEModel::getFilesInUse( $this->selected_store_id, $this->selected_language_id )
																);
...
class FOOTERSCHEDULEModel extends ObjectModel
{

	public static $definition = array(
		'table' => 'footerschedule',
		'primary' => 'file_id',
		'multishop' => true,
		'multilang' => true,
		'fields' => array(
	...
	public static function createTables()
	{
		...
		return ( FOOTERSCHEDULEModel::createFilesTable()
				&& FOOTERSCHEDULEModel::createContentTable());
	}
