...
<div class="col-sm-3 custom-cart clearfix{if $PS_CATALOG_MODE} header_user_catalog{/if}">
{/if}
	<div class="shopping_cart">
		<a href="{$link->getPageLink($order_process, true)|escape:'html':'UTF-8'}" title="{l s='View my shopping cart' mod='blockcart'}" rel="nofollow">
			<b>{l s='В корзине' mod='blockcart'}</b>
			<span class="ajax_cart_quantity{if $cart_qties == 0} unvisible{/if}">{$cart_qties}</span>
			<span class="ajax_cart_product_txt{if $cart_qties != 1} unvisible{/if}">{l s='товар' mod='blockcart'}</span>
			<span class="ajax_cart_product_txt_s cart-flag {if $cart_qties < 2} unvisible{/if}">{if $cart_qties < 5}{l s='товара' mod='blockcart'}{/if}{if $cart_qties > 4}{l s='товаров' mod='blockcart'}{/if}</span>
			...
			<div class="cart_block block exclusive">
				<div class="block_content">
					...
					<div class="cart_block_list{if isset($blockcart_top) && !$blockcart_top}{if isset($colapseExpandStatus) && $colapseExpandStatus eq 'expanded' || !$ajax_allowed || !isset($colapseExpandStatus)} expanded{else} collapsed unvisible{/if}{/if}">
						...
						<br clear="all" />
						<a id="quickorder" href="#" rel="nofollow" title="{l s='Quick order' mod='quickorder'}" class="exclusive_large"><span></span>{l s='Быстрый заказ' mod='quickorder'}</a>
							...
							<a id="button_order_cart" class="btn btn-default button button-small" href="{$link->getPageLink("$order_process", true)|escape:"html":"UTF-8"}" title="{l s='Check out' mod='blockcart'}" rel="nofollow">
								<span>
									{l s='Перейти в корзину' mod='blockcart'}</i>
								</span>
							</a>
						</p>
					</div>
	...
	<span class="ajax_block_products_total">
		...
		{/if}
				...
				<div class="button-container">
					...
					{*	</span>*}
					{*</a>*}
					<a class="btn btn-default button button-medium"	href="{$link->getPageLink("$order_process", true)|escape:"html":"UTF-8"}" title="{l s='Proceed to checkout' mod='blockcart'}" rel="nofollow" onclick="$.fancybox.close();$('#quickorder').click();return false;">
						<span>
							{l s='Proceed to checkout' mod='blockcart'}<i class="icon-chevron-right right"></i>
						</span>
					</a>
