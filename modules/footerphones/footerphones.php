...
class FOOTERPHONES extends Module
{
	public function __construct()
	{
		$this->name = 'footerphones';
		$this->description = 'footerPhones';
		$this->tab = 'front_office_features';
		$this->version = '1.1.1';
		$this->author = 'emotionLoop';
		$this->need_instance = 0;
		$this->ps_versions_compliancy = array('min' => '1.5', 'max' => '1.6');
		$this->monolanguage_content = 'Footerphones_MONOLANGUAGE';
		$this->text_editor_content = 'Footerphones_TEXTEDITOR';
		$this->content_wrapper = 'Footerphones_CONTENTWRAPPER';
		$this->content_wrapper_class = 'Footerphones_CONTENTWRAPPER_CLASS';
		$this->content_wrapper_id = 'Footerphones_CONTENTWRAPPER_ID';
		$this->bootstrap = true;
		$this->_html = '';
		...
		$this->displayName = $this->l('footerPhones');
		$this->description = $this->l('footerPhones');
	...
	public function install()
	{
		...
		if (!parent::install() || !$this->registerHook('header') || !$this->registerHook('footer') || !FOOTERPHONESModel::createTables())
			return false;
	...
	public function addFilesToTemplate($path = null)
	{
		...
		$files_data = FOOTERPHONESModel::getFilesInUse( $this->selected_store_id, $this->selected_language_id );
	...
	{
		$content_query = FOOTERPHONESModel::getContent( $this->selected_store_id, $this->selected_language_id );
		$pre_content = '';
		$pos_content = '';
		...
		if (Tools::isSubmit('submit'.$this->name))
		{
				...
				&& (int)Configuration::get('PS_LANG_DEFAULT') == $this->selected_language_id ))
			{
				...
				if (Tools::getValue('content_text') !== false && $this->ignore_changes_content_changes == false)
					FOOTERPHONESModel::setContent( Tools::getValue('content_text'), $this->selected_store_id, $this->selected_language_id );

				//store the files to be used
				if (Tools::getValue('headerFiles') !== false)
					FOOTERPHONESModel::setFiles(
	...
	public function displayForm()
	{
		...
		$helper->fields_value['headerFiles[]'] = $this->processSelectFilesForMultiselect(
																FOOTERPHONESModel::getFilesInUse( $this->selected_store_id, $this->selected_language_id )
																);
...
class FOOTERPHONESModel extends ObjectModel
{
	...
	public static function createTables()
	{
		...
		return ( FOOTERPHONESModel::createFilesTable()
				&& FOOTERPHONESModel::createContentTable());
	}
