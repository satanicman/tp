...
class FOOTERSOCICONS extends Module
{
	public function __construct()
	{
		$this->name = 'footersocicons';
		$this->description = 'footerSocIcons';
		$this->tab = 'front_office_features';
		$this->version = '1.1.1';
		$this->author = 'emotionLoop';
		$this->need_instance = 0;
		$this->ps_versions_compliancy = array('min' => '1.5', 'max' => '1.6');
		$this->monolanguage_content = 'Footersocicons_MONOLANGUAGE';
		$this->text_editor_content = 'Footersocicons_TEXTEDITOR';
		$this->content_wrapper = 'Footersocicons_CONTENTWRAPPER';
		$this->content_wrapper_class = 'Footersocicons_CONTENTWRAPPER_CLASS';
		$this->content_wrapper_id = 'Footersocicons_CONTENTWRAPPER_ID';
		$this->bootstrap = true;
		$this->_html = '';
		...
		$this->displayName = $this->l('footerSocIcons');
		$this->description = $this->l('footerSocIcons');
	...
	public function install()
	{
		...
		if (!parent::install() || !$this->registerHook('header') || !$this->registerHook('footer') || !FOOTERSOCICONSModel::createTables())
			return false;
	...
	public function uninstall()
	{
		$this->_clearCache('template.tpl');
		if (!parent::uninstall()
			|| !FOOTERSOCICONSModel::DropTables())
		{
	...
	public function addFilesToTemplate($path = null)
	{
		...
		$files_data = FOOTERSOCICONSModel::getFilesInUse( $this->selected_store_id, $this->selected_language_id );
	...
	{
		$content_query = FOOTERSOCICONSModel::getContent( $this->selected_store_id, $this->selected_language_id );
		$pre_content = '';
		$pos_content = '';
		...
		if (Tools::isSubmit('submit'.$this->name))
		{
				...
				&& (int)Configuration::get('PS_LANG_DEFAULT') == $this->selected_language_id ))
			{
				...
				if (Tools::getValue('content_text') !== false && $this->ignore_changes_content_changes == false)
					FOOTERSOCICONSModel::setContent( Tools::getValue('content_text'), $this->selected_store_id, $this->selected_language_id );

				//store the files to be used
				if (Tools::getValue('headerFiles') !== false)
					FOOTERSOCICONSModel::setFiles(
	...
	public function displayForm()
	{
		...
		$helper->fields_value['headerFiles[]'] = $this->processSelectFilesForMultiselect(
																FOOTERSOCICONSModel::getFilesInUse( $this->selected_store_id, $this->selected_language_id )
																);
...
class FOOTERSOCICONSModel extends ObjectModel
{

	public static $definition = array(
		'table' => 'footersocicons',
		'primary' => 'file_id',
		'multishop' => true,
		'multilang' => true,
		'fields' => array(
	...
	public static function createTables()
	{
		...
		return ( FOOTERSOCICONSModel::createFilesTable()
				&& FOOTERSOCICONSModel::createContentTable());
	}
