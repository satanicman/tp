...
{capture name=path}{l s='Ваша корзина'}{/capture}

<h1 id="cart_title" class="page-heading">{l s='Ваша корзина'}
	{if !isset($empty) && !$PS_CATALOG_MODE}
		<span class="heading-counter">{l s='В Вашей корзине:'}
			<span id="summary_products_quantity">{$productNumber} {if $productNumber == 1}{l s='товар'}{elseif $productNumber < 5}{l s='товара '}{else}{l s='товаров'}{/if}</span>
		</span>
	{/if}
</h1>
...
{else}
	<p id="emptyCartWarning" class="alert alert-warning unvisible">{l s='Ваша корзина пуста.'}</p>
	...
	<div id="order-detail-content" class="table_block table-responsive">
		<table id="cart_summary" class="table table-bordered {if $PS_STOCK_MANAGEMENT}stock-management-on{else}stock-management-off{/if}">
			<thead>
				<tr>
					<th class="cart_product first_item">{l s='Продукт'}</th>
					<th class="cart_description item">{l s='Описание'}</th>
					{if $PS_STOCK_MANAGEMENT}
						{assign var='col_span_subtotal' value='3'}
						<th class="cart_avail item text-center">{l s='Доступность'}</th>
					{else}
						{assign var='col_span_subtotal' value='2'}
					{/if}
					<th class="cart_unit item text-right">{l s='Цена за ед.'}</th>
					<th class="cart_quantity item text-center">{l s='Кол-во'}</th>
					<th class="cart_delete last_item">&nbsp;</th>
					<th class="cart_total item text-right">{l s='Всего'}</th>
				</tr>
			</thead>
			<tfoot>
				...
				{else}
					<tr class="cart_total_price">
						...
						{* <td colspan="{$col_span_subtotal}" class="text-right">{l s='Общая за товар'}</td> *}
						{* <td colspan="2" class="price" id="total_product">{displayPrice price=$total_products}</td> *}
					</tr>
				{/if}
				...
				{*		<td colspan="{$col_span_subtotal}" class="text-right">{l s='Общая за доставку'}</td>*}
				{*		<td colspan="2" class="price" id="total_shipping">{l s='Бесплатная доставка!'}</td>*}
				{*	</tr>*}
				{*{else}*}
				{*	{if $use_taxes && $total_shipping_tax_exc != $total_shipping}*}
				{*		{if $priceDisplay}*}
				{*			<tr class="cart_total_delivery{if $total_shipping_tax_exc <= 0} unvisible{/if}">*}
				{*				<td colspan="{$col_span_subtotal}" class="text-right">{if $display_tax_label}{l s='Общая за доставку (tax excl.)'}{else}{l s='Общая за доставку'}{/if}</td>*}
				...
				{*				<td colspan="{$col_span_subtotal}" class="text-right">{if $display_tax_label}{l s='Общая за доставку (tax incl.)'}{else}{l s='Общая за доставку'}{/if}</td>*}
	...
	<p class="cart_navigation clearfix">
		...
		<a href="{if (isset($smarty.server.HTTP_REFERER) && ($smarty.server.HTTP_REFERER == $link->getPageLink('order', true) || $smarty.server.HTTP_REFERER == $link->getPageLink('order-opc', true) || strstr($smarty.server.HTTP_REFERER, 'step='))) || !isset($smarty.server.HTTP_REFERER)}{$link->getPageLink('index')}{else}{$smarty.server.HTTP_REFERER|escape:'html':'UTF-8'|secureReferrer}{/if}" class="button-exclusive btn btn-default" title="{l s='Continue shopping'}">
			<i class="icon-chevron-left"></i>{l s='Продолжить покупки'}
		</a>
	</p>

	<a class="last-btn btn btn-default button button-medium"
        href="{$link->getPageLink("$order_process", true)|escape:"html":"UTF-8"}"
        title="{l s='Оформить заказ' mod='blockcart'}"
        rel="nofollow"
        onclick="$.fancybox.close();$('#quickorder').click();return false;">
      <span>
