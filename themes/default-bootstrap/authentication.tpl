...
*}
{capture name=path}
	{if !isset($email_create)}{l s='Аутентификация'}{else}
		<a href="{$link->getPageLink('authentication', true)|escape:'html':'UTF-8'}" rel="nofollow" title="{l s='Аутентификация'}">{l s='Аутентификация'}</a>
		<span class="navigation-pipe">{$navigationPipe}</span>{l s='Создать аккаунт'}
	{/if}
{/capture}
<h1 class="page-heading">{if !isset($email_create)}{l s='Аутентификация'}{else}{l s='Создать аккаунт'}{/if}</h1>
	...
	<div class="row">
		<div class="col-xs-12 col-sm-6">
			<form action="{$link->getPageLink('authentication', true)|escape:'html':'UTF-8'}" method="post" id="create-account_form" class="box">
				<h3 class="page-subheading">{l s='Создать аккаунт'}</h3>
				<div class="form_content clearfix">
					<p>{l s='Введите e-mail для создания аккаунта'}</p>
					<div class="alert alert-danger" id="create_account_error" style="display:none"></div>
					<div class="form-group">
						<label for="email_create">{l s='Адрес e-mail'}</label>
		...
		<div class="col-xs-12 col-sm-6">
			<form action="{$link->getPageLink('authentication', true)|escape:'html':'UTF-8'}" method="post" id="login_form" class="box">
				<h3 class="page-subheading">{l s='Уже зарегистрированы?'}</h3>
				<div class="form_content clearfix">
					<div class="form-group">
		...
		<form action="{$link->getPageLink('authentication', true, NULL, "back=$back")|escape:'html':'UTF-8'}" method="post" id="new_account_form" class="std clearfix">
			<div class="box">
				<div id="opc_account_form" style="display: block; ">
					<h3 class="page-heading bottom-indent">{l s='Мгновенный заказ'}</h3>
					<p class="required"><sup>*</sup>{l s='Обязательное поле'}</p>
					<!-- Account -->
					<div class="required form-group">
						<label for="guest_email">{l s='Адрес e-mail'} <sup>*</sup></label>
					...
					<div class="required form-group">
						<label for="firstname">{l s='Фамилия'} <sup>*</sup></label>
					...
					<div class="required form-group">
						<label for="lastname">{l s='Имя'} <sup>*</sup></label>
					...
					{if isset($newsletter) && $newsletter}
						<div class="checkbox">
							...
							{l s='Подписаться на новости!'}</label>
						</div>
					{/if}
					{if isset($optin) && $optin}
						<div class="checkbox">
							...
							{l s='Получать специальные предложения!'}</label>
						</div>
					{/if}
					...
					{foreach from=$dlv_all_fields item=field_name}
						...
						{elseif $field_name eq "address2"}
							<div class="form-group is_customer_param">
								<label for="address2">{l s='Адрес (Line 2)'}{if in_array($field_name, $required_fields)} <sup>*</sup>{/if}</label>
						...
						{elseif $field_name eq "postcode"}
							...
							<div class="required postcode form-group">
								<label for="postcode">{l s='Почтовый код'} <sup>*</sup></label>
						...
						{elseif $field_name eq "State:name"}
							...
							<div class="required id_state select form-group">
								<label for="id_state">{l s='Регион'} <sup>*</sup></label>
					...
					<div class="checkbox">
						...
						{l s='Использовать другой адрес для счета'}</label>
					</div>
					<div id="opc_invoice_address"  class="unvisible">
						{assign var=stateExist value=false}
						{assign var=postCodeExist value=false}
						{assign var=dniExist value=false}
						<h3 class="page-subheading top-indent">{l s='Адрес для счета'}</h3>
						...
						<div class="required form-group">
							<label for="firstname_invoice">{l s='Фамилия'} <sup>*</sup></label>
						...
						<div class="required form-group">
							<label for="lastname_invoice">{l s='Имя'} <sup>*</sup></label>
						...
						{elseif $field_name eq "address1"}
						<div class="required form-group">
							<label for="address1_invoice">{l s='Адрес'} <sup>*</sup></label>
						...
						{elseif $field_name eq "address2"}
						<div class="form-group is_customer_param">
							<label for="address2_invoice">{l s='Адрес (Line 2)'}{if in_array($field_name, $required_fields)} <sup>*</sup>{/if}</label>
						...
						<div class="required postcode_invoice form-group">
							<label for="postcode_invoice">{l s='Почтовый код'} <sup>*</sup></label>
						...
						{elseif $field_name eq "city"}
						<div class="required form-group">
							<label for="city_invoice">{l s='Город'} <sup>*</sup></label>
						...
						<div class="required form-group">
							<label for="id_country_invoice">{l s='Страна'} <sup>*</sup></label>
						...
						<div class="required id_state_invoice form-group" style="display:none;">
							<label for="id_state_invoice">{l s='Регион'} <sup>*</sup></label>
						...
						{/if}
						<div class="form-group is_customer_param">
							<label for="other_invoice">{l s='Дополнительная информация'}</label>
							<textarea class="form-control" name="other_invoice" id="other_invoice" cols="26" rows="3"></textarea>
						</div>
						{if isset($one_phone_at_least) && $one_phone_at_least}
							<p class="inline-infos required is_customer_param">{l s='Нужен хотя бы один номер телефона.'}</p>
						{/if}
						<div class="form-group is_customer_param">
							<label for="phone_invoice">{l s='Домашний телефон'}</label>
			...
			<p class="cart_navigation required submit clearfix">
				<span><sup>*</sup>{l s='Обязательное поле'}</span>
...
{else}
	...
	<form action="{$link->getPageLink('authentication', true)|escape:'html':'UTF-8'}" method="post" id="account-creation_form" class="std box">
		...
		<div class="account_creation">
			<h3 class="page-subheading">{l s='Персональная информация'}</h3>
			<p class="required"><sup>*</sup>{l s='Обязательное поле'}</p>
			<div class="required form-group">
				<label for="customer_firstname">{l s='Фамилия'} <sup>*</sup></label>
			...
			<div class="required form-group">
				<label for="customer_lastname">{l s='Имя'} <sup>*</sup></label>
			...
			<div class="required form-group">
				<label for="phone">{l s='Телефон'} <sup>*</sup></label>
				<input type="text" class="is_required validate form-control phone_mask" data-validate="isPhoneNumber" id="phone" name="phone" value="{if isset($smarty.post.phone)}{$smarty.post.phone}{/if}" />
			    {literal}
                    <script>
			...
			<div class="required form-group">
				<label for="address">{l s='Адрес'} <sup>*</sup></label>
				<textarea type="text" class="is_required validate form-control" data-validate="isAddress" id="address" name="address">{if isset($smarty.post.address)}{$smarty.post.address}{/if}</textarea>
			</div>
			<div class="required form-group">
			...
			<div class="required password form-group">
				<label for="passwd">{l s='Пароль'} <sup>*</sup></label>
				<input type="password" class="is_required validate form-control" data-validate="isPasswd" name="passwd" id="passwd" />
				<span class="form_info">{l s='(Минимум пять символов)'}</span>
			</div>
			<div class="form-group">
				<label>{l s='Дата рождения'}</label>
				<div class="row">
			...
			{if isset($newsletter) && $newsletter}
				<div class="checkbox">
					...
					<label for="newsletter">{l s='Подписаться на новости!'}</label>
			...
			{if isset($optin) && $optin}
				<div class="checkbox">
					...
					<label for="optin">{l s='Получать предложения от партнеров!'}</label>
		...
		{if $b2b_enable}
			<div class="account_creation">
				<h3 class="page-subheading">{l s='Информация о Вашей компании'}</h3>
				<p class="form-group">
					<label for="">{l s='Компания'}</label>
					<input type="text" class="form-control" id="company" name="company" value="{if isset($smarty.post.company)}{$smarty.post.company}{/if}" />
				</p>
				<p class="form-group">
		...
		{if isset($PS_REGISTRATION_PROCESS_TYPE) && $PS_REGISTRATION_PROCESS_TYPE}
			<div class="account_creation">
				<h3 class="page-subheading">{l s='Ваш адрес'}</h3>
				{foreach from=$dlv_all_fields item=field_name}
					{if $field_name eq "company"}
						{if !$b2b_enable}
							<p class="form-group">
								<label for="company">{l s='Компания'}{if in_array($field_name, $required_fields)} <sup>*</sup>{/if}</label>
								<input type="text" class="form-control" id="company" name="company" value="{if isset($smarty.post.company)}{$smarty.post.company}{/if}" />
							</p>
						{/if}
					{elseif $field_name eq "vat_number"}
				...
				<p class="form-group">
					<label for="phone">{l s='Домашний телефон'}{if isset($one_phone_at_least) && $one_phone_at_least} <sup>**</sup>{/if}</label>
				...
				<p class="{if isset($one_phone_at_least) && $one_phone_at_least}required {/if}form-group">
					<label for="phone_mobile">{l s='Мобильный телефон'}{if isset($one_phone_at_least) && $one_phone_at_least} <sup>**</sup>{/if}</label>
					<input type="text" class="form-control" name="phone_mobile" id="phone_mobile" value="{if isset($smarty.post.phone_mobile)}{$smarty.post.phone_mobile}{/if}" />
				</p>
				{if isset($one_phone_at_least) && $one_phone_at_least}
					{assign var="atLeastOneExists" value=true}
					<p class="inline-infos required">** {l s='Нужен хотя бы один номер телефона.'}</p>
				{/if}
		...
		<div class="submit clearfix">
			...
			<button type="submit" name="submitAccount" id="submitAccount" class="btn btn-default button button-medium">
				<span>{l s='Зарегистрироваться'}<i class="icon-chevron-right right"></i></span>
			</button>
			<p class="pull-right required"><span><sup>*</sup>{l s='Обязательное поле'}</span></p>
		</div>
	</form>
{/if}
{strip}
