...
<div itemscope itemtype="https://schema.org/Product">
	...
	<h2 itemprop="name" class="b1c-name hidden">{$product->name|escape:'html':'UTF-8'}</h2>
		{if !$content_only}
			<div class="container">
		...
		<div class="pb-left-column col-xs-12 col-sm-4 col-md-5">
			...
			<div id="image-block" class="clearfix">
				{if $product->new}
					<span class="new-box">
						<span class="new-label">{l s='Новинка'}</span>
					</span>
				{/if}
				{if $product->on_sale}
		...
		<div class="pb-center-column col-xs-12 col-sm-4">
			...
			<p id="availability_statut"{if !$PS_STOCK_MANAGEMENT || ($product->quantity <= 0 && !$product->available_later && $allow_oosp) || ($product->quantity > 0 && !$product->available_now) || !$product->available_for_order || $PS_CATALOG_MODE} style="display: none;"{/if} class="{if $product->quantity <= 0 && !$allow_oosp} pr-yellow{elseif $product->quantity <= 0} pr-red{else} pr-green{/if}">
				{*<span id="availability_label">{l s='Availability:'}</span>*}
				{*<span id="availability_value" class="label{if $product->quantity <= 0 && !$allow_oosp} label-danger{elseif $product->quantity <= 0} label-warning{else} label-success{/if}">{if $product->quantity <= 0}{if $PS_STOCK_MANAGEMENT && $allow_oosp}{$product->available_later}{else}{l s='This product is no longer in stock'}{/if}{elseif $PS_STOCK_MANAGEMENT}{l s='Есть в наличии'}{/if}</span>*}
				{if $product->quantity <= 0 && !$allow_oosp} Товар ожидается{elseif $product->quantity <= 0} Товар отсутствует{else} Есть в наличии{/if}
			</p>

			<p>
				<label>{l s='Код товара:'} </label>
				<span>{$product->id_oneC}</span>
			</p>
			<p id="product_reference"{if empty($product->reference) || !$product->reference} style="display: none;"{/if}>
				<label>{l s='Артикул:'} </label>
				<span class="editable" itemprop="sku"{if !empty($product->reference) && $product->reference} content="{$product->reference}"{/if}>{if !isset($groups)}{$product->reference|escape:'html':'UTF-8'}{/if}</span>
			</p>
<!-- href="{$product->id}-{$product->link_rewrite|escape:'html':'UTF-8'}" -->
			<div class="compare">
				<a class="add_to_compare" style="    cursor: pointer;" data-id-product="{$product->id}">Добавить к сравнению<i class="plus"></i></a>
				<div id="add_to_compare_after">{include file="./product-compare.tpl"}</div>
			</div>
					...
					<div class="product_attributes clearfix">
						...
						<p id="quantity_wanted_p"{if (!$allow_oosp && $product->quantity <= 0) || !$product->available_for_order || $PS_CATALOG_MODE} style="display: none;"{/if}>
							...
							<input type="" min="1" name="qty" id="quantity_wanted" class="text" value="{if isset($quantityBackup)}{$quantityBackup|intval}{else}{if $product->minimal_quantity > 1}{$product->minimal_quantity}{else}1{/if}{/if}" />
...
{**}
		...
		{*</p>*}
				...
				<div class="box-info-product">
					<div class="box-cart-bottom">
						...
						<div style="float: right;" {if (!$allow_oosp && $product->quantity <= 0) || !$product->available_for_order || (isset($restricted_country_mode) && $restricted_country_mode) || $PS_CATALOG_MODE} class="unvisible"{/if}>
							<p id="b1c" class="buttons_bottom_block no-print">
								<button class="one-cl exclusive b1c">
			...
			<div class="prod-soc-icons a2a_kit">
				<span>Поделиться</span>
				<a class="a2a_button_facebook"><i class="fa fa-facebook" aria-hidden="true"></i></a>
				<a class="a2a_button_twitter"><i class="fa fa-twitter" aria-hidden="true"></i></a>
				{*<a class="a2a_button_tumblr"><i class="fa fa-tumblr" aria-hidden="true"></i></a>*}
		...
		<a href="/content/8-dostavka-i-oplata"><div class="product-info-block">
			<div>
				<h3>Доставка</h3>
				<div class="pr-inf-bl-one">
					<img src="../../../img/restaurant.png" />
					<p>Самовывоз из магазина</p>
				</div>
				<div class="pr-inf-bl-one">
					<img src="../../../img/Mensajero.png" />
					<p>Курьером по городу</p>
				</div>
				<div class="pr-inf-bl-one">
					<img src="../../../img/delivery.png" />
					<p>по Украине: согласно тарифам перевозчика</p>
				</div>
			</div>
			<div>
				<h3>Оплата</h3>
				<div class="pr-inf-bl-one">
					...
					<p>Наличными (курьеру, в магазине при самовывозе, наложенный платеж)</p>
				</div>
				<div class="pr-inf-bl-one">
					<img src="../../../img/business.png" />
					<p>Оплата через платежную систему Портмоне, Приват24 или банковский перевод</p>
				</div>
			</div>
		</div></a>
...
{if (isset($quantity_discounts) && count($quantity_discounts) > 0)}
		...
		<div class="col-sm-7 col-xs-12">
			<div class="tab-heads">
				<h3 id="tab-head-2" class="tab-head page-product-heading">Описание</h3>
				<h3 id="tab-head-1" class="tab-head page-product-heading">Характеристики</h3>
				{*<h3 id="tab-head-3" class="tab-head page-product-heading">Отзывы</h3>*}
			</div>
			{if isset($features) && $features}
				<!-- Data sheet -->
				<section id="prod-cont-1" class="page-product-box">
			...
			{if isset($product) && $product->description}
				<!-- More info -->
				<section id="prod-cont-2" class="page-product-box">
					<!-- full description -->
		...
		<div class="col-sm-5">
					<!--HOOK_PRODUCT_TAB -->
			<section id="prod-cont-3" class="page-product-box">
				{$HOOK_PRODUCT_TAB}
